...
class CheckingAccount extends BankingAccount_1.default {
  ...
  deposit(amount) {
    if (amount < 0) {
      ...
      this.balance += amount;
      console.log(
        `Deposited ${amount} to the checking account: ${this.getAccountNumber()}. New balance: ${this.getBalance()}`,
      );
    }
  }
  withdraw(amount) {
    if (amount < 0 || amount > this.getBalance() - 50) {
      ...
      this.balance -= amount;
      console.log(
        `Withdrew ${amount} from the checking account: ${this.getAccountNumber()}. New balance: ${this.getBalance()}`,
      );
    }
  }
  getBalance() {
